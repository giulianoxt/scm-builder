...
require_once("Logicamente.class.php");
	...
	if (isset($_POST['action'])){
		switch ($_POST['action']){
			...
			case "readFormula"     : serializeTree($_POST['formula']);			   break;
    ...
    function serializeTree($formula) {
        $form = str_replace("^","&",$formula);
        $logica = new Logicamente();               
        $node = $logica->readFormula($form, unserialize($_SESSION['con']));       
        $cont = $_POST['cont'];
